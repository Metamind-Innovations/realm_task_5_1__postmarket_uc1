...
def get_kernel():
    """Create circular kernel for lung boundary morphological operations.

    Returns:
        np.ndarray: 11x11 binary kernel with circular structure
    """
...
def create_flipped_mask(
    image, i_orig, kernel, method=0
):  # image is a lung mask image, i_orig is an image volume
    ...
    if method:
        ...
        for i in range(min_bound, min_bound + 5):
            ...
            lungs_center = (
                int(rib_view.shape[0] / 2 - c_limit)
                + np.where(
                    filtered_distr[
                        int(rib_view.shape[0] / 2 - c_limit) : int(
                            rib_view.shape[0] / 2 + c_limit
                        )
                    ]
                    == np.max(
                        filtered_distr[
    ...
    for i in range(min_bound, max_bound + 1):
        ...
        if lungs_center_ind[i] > int(image.shape[2] / 2):
            part1 = ax[
                :, 2 * lungs_center_ind[i] - image.shape[2] : lungs_center_ind[i]
            ]
            part2 = np.hstack(
                (
                    ax[:, lungs_center_ind[i] :],
                    np.zeros(
                        (image.shape[1], 2 * lungs_center_ind[i] - image.shape[2])
                    ),
                )
            )
            ...
            fl_mask[i, ...] = np.hstack(
                (np.flip(part2, axis=1), np.flip(part1, axis=1))
            )
            ...
            part1 = np.hstack(
                (
                    np.zeros(
                        (image.shape[1], image.shape[2] - 2 * lungs_center_ind[i])
                    ),
                    ax[:, : lungs_center_ind[i]],
                )
            )
            part2 = ax[
                :,
                lungs_center_ind[i] : image.shape[2]
                - (image.shape[2] - 2 * lungs_center_ind[i]),
            ]
            part2 = cv2.dilate(np.array(part2, np.uint8), kernel, iterations=1)
            part1 = cv2.dilate(np.array(part1, np.uint8), kernel, iterations=1)
            fl_mask[i, ...] = np.hstack(
...
def parse_dataset(general_path, img_only=True):
    ...
    for patient in tqdm(patients):
        for root, dirs, files in os.walk(os.path.join(general_path, patient)):
            for file in files:
                if not img_only:
                    # Specifically look for GTV-1 mask
                    if "gtv-1_mask" in file.lower():
                        temp_mask = file
                        mask_path = os.path.join(general_path, patient, temp_mask)
                        print(f"Found GTV mask: {mask_path}")
                        for imgfile in files:
                            if re.search("volume", imgfile.lower()) or re.search(
                                "image", imgfile.lower()
                            ):
                                temp_img = imgfile
                                Patient_dict[i] = [
                                    os.path.join(general_path, patient, temp_img),
                                    mask_path,
                                ]
                                i += 1
                                temp_mask = ""
                    ...
                    if re.search("volume", file.lower()) or re.search(
                        "image", file.lower()
                    ):
                        temp_img = file
                        Patient_dict[i] = [
    ...
    print(len(Patient_dict), " Patients with GTV-1 masks found")
    return Patient_dict
