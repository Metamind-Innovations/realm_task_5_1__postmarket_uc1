...
class Patient_data_generator(keras.utils.Sequence):
    ...
    def _load_patient(self, filename):
        ...
        if self.verbosity:
            print(
                "-" * 7,
                "Img spacing ",
                sitk.ReadImage(self.patient_dict[filename][0]).GetSpacing(),
                "-" * 7,
            )
            print("Image shape:", img.shape)

        # Set minimal values for HU intensities
        if self.hu_intensity:  # Set air threshold to -1000 HU (Hounsfield Units)
            img[img < -1000] = -1000
            if self.verbosity:
        ...
        if self.extract_lungs:
            ...
            if temp_args is not None:
                ...
                min_ind_l_mask, max_ind_l_mask = (
                    np.min(lung_mask_indicies),
                    np.max(lung_mask_indicies),
                )
                try:
                    tum_mask_indicies = np.where(np.sum(msk, axis=(1, 2)) != 0)
                    min_ind_tum_mask, max_ind_tum_mask = (
                        np.min(tum_mask_indicies),
                        np.max(tum_mask_indicies),
                    )
                except:
                ...
                for k, layer in enumerate(msk):
                    ...
                    lung_mask_with_tumor_mask[k, ...] = np.array(
                        lung_mask_with_tumor_mask[k, ...], np.uint8
                    ) | cv2.dilate(np.array(temp_layer, np.uint8), kernel, iterations=3)
                ...
                print(
                    "Lungs are not extracted for patient",
                    filename[0][:-11].split("\\")[-1],
                )
                pass
        ...
        if True:
            ...
            new_img = np.array(
                self._resize_3d_img(
                    ...
                    (
                        ...
                        int(temp_data_orig.GetSpacing()[1] * img.shape[2]),
                    ),
                    cv2.INTER_NEAREST,
                ),
                np.int16,
            ...
            if new_img.shape[1] > 512:
                ...
                new_img_temp = (
                    np.ones(
                        (new_img.shape[0], self.image_size, self.image_size), np.int16
                    )
                    * -1000
                )
    ...
    def _load_patient_for_predict(self, filename, eval_mode=False, img_type=np.int16):
        ...
        image_transform_params = {
            ...
            "z_st": None,
            "z_end": None,
            "img_origin": temp_data.GetOrigin(),
            "original_spacing": temp_data.GetSpacing(),
        }
        ...
        if self.reshape and not eval_mode:
            new_shape = (
                int(img.shape[0]),
                int(temp_data.GetSpacing()[0] * img.shape[1]),
                int(temp_data.GetSpacing()[1] * img.shape[2]),
            )

            new_img = np.array(
                le.resize_3d_img(img, new_shape, cv2.INTER_NEAREST), np.int16
            )
            if not self.img_only:
                msk = (
                    np.array(
                        le.resize_3d_img(msk, new_shape, cv2.INTER_NEAREST), np.uint8
                    )
                    > 0
                )
            ...
            if new_img.shape[1] > self.image_size:
                ...
                if not self.img_only:
                    msk_temp = np.zeros(
                        (new_img.shape[0], self.image_size, self.image_size), np.uint8
                    )
        ...
        if self.predict:
            ...
            if self.calculate_snr:
                ...
                return (
                    imgs,
                    msks,
                    [self.patient_dict[filename][0] for filename in filenames],
                    snrs,
                    params,
                )
            else:
        ...
        # train mode: return images and masks
            ...
            while len(zero_arr) < int(self.batch_size * self.zero_sl_ratio):
                if self.__epoch_zero_dict[zero_key]:
                    ...
                    zero_arr.append(
                        (t_img[temp_zero_ind, ...], t_msk[temp_zero_ind, ...])
                    )  # zeroslices
                else:
            ...
            while len(arr) < self.batch_size - int(
                self.batch_size * self.zero_sl_ratio
            ):
    ...
    def _normalization_init(self):
        if self.normalize and len(self.norm_coeff) != 2 and not self.predict:
            ...
            mean = np.mean(
                [
                    np.mean(
                        self._load_patient_for_predict(filename, eval_mode=True)[
                            0
                        ].flatten()
                    )
        ...
        if not self.predict:
            ...
            for pat in tqdm(self.filenames):
                ...
                if (
                    ...
                    and int(temp_img.GetSpacing()[2]) != self._slice_thickness
                ):
                    pass
                else:
                    z_tumor_distr = np.sum(temp_msk, axis=(1, 2))
                    tum_indicies = np.where(
                        z_tumor_distr
                        > self._tum_size / np.prod([*temp_msk_im.GetSpacing()][:2])
                    )[0]
                    ...
                    if size_eval:
                        temp_distr = list(
                            filter(
                                ...
                                np.round(
                                    z_tumor_distr
                                    * np.prod([*temp_msk_im.GetSpacing()][:2]),
                                    decimals=1,
                                ),
                            )
                        )  # mm^2
                        ...
                        self.tum_volume_distr[
                            self.patient_dict[pat][0].split("\\")[-2]
                        ] = np.round(
                            np.sum(z_tumor_distr)
                            * temp_msk_im.GetSpacing()[0]
                            * temp_msk_im.GetSpacing()[1]
                            * temp_msk_im.GetSpacing()[2]
                            * 0.001,
                            2,
                        )  # ml
            ...
            print("-" * 7, "Loading patients in predict mode", "-" * 7)
    ...
    def __len__(self):
        if self.predict:
            ...
            len_zero_dict = np.sum(
                [len(self.zero_dict[x]) for x in self.zero_dict.keys()]
            )
            if self.verbosity:
                print(
                    "-" * 7,
                    "Generator contained %d tumor slices and %d empty slices"
                    % (len_tum_dict, len_zero_dict),
                    "-" * 7,
                )
                ...
                [
                    int(
                        ...
                        / (
                            0.001
                            + self.batch_size
                            - int(self.batch_size * self.zero_sl_ratio)
                        )
                    ),
                    int(
                        len_zero_dict
                        / (0.001 + int(self.batch_size * self.zero_sl_ratio))
                    ),
                ]
            )
