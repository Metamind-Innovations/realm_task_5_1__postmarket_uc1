    ...
    def __read_files(self):
        ...
        if ".csv" in self._feature_path:
            feature_df = pd.read_csv(
                self._feature_path, dtype={self._patient_column: str}
            )
        ...
        if len(self._feature_column) > 0:
            self._feature_column = list(
                set(self._feature_column) & set(list(feature_df.columns))
            )
        else:
        ...
        if len(self._outcome_path) > 0:
            if (len(self._patient_column) > 0) & (
                len(self._patient_in_outcome_column) > 0
            ):
                if ".csv" in self._outcome_path:
                    outcome_df = pd.read_csv(
                        self._outcome_path,
                        dtype={
                            self._patient_in_outcome_column: str,
            ...
            for patient_outcome in list(self._outcome.index):
                for patient in self._patient_name:
                    if patient_outcome in patient:
                        self._feature_outcome_dataframe.at[
                            patient, self._outcome_column
                        ] = self._outcome[patient_outcome]
            self._outcome = self._feature_outcome_dataframe[self._outcome_column]
            self._class_label = pd.unique(
                np.array(list(self._feature_outcome_dataframe[self._outcome_column]))
            )
