...
class ResultSet:
    ...
    def __create_dataframes(self):
        ...
        if len(self._train_labels) and len(self._train_labels) == len(
            self._train_predictions
        ):
            self.train_df = pd.DataFrame(
                {"labels": self._train_labels, "predictions": self._train_predictions}
            )
        ...
        if len(self._test_labels) and len(self._test_labels) == len(
            self._test_predictions
        ):
            self.test_df = pd.DataFrame(
                {"labels": self._test_labels, "predictions": self._test_predictions}
            )
        elif len(self._test_labels) != len(self._test_predictions):
            warn("length of test labels and predictions don't match")
        if len(self._external_labels) and len(self._external_labels) == len(
            self._external_predictions
        ):
            self.external_df = pd.DataFrame(
                {
                    "labels": self._external_labels,
                    "predictions": self._external_predictions,
                }
            )
