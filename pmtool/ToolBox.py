...
class ToolBox(DataSet):
                ...
                for i, temp_slice in enumerate(image):
                    new_row = pd.Series(
                        [
                            pat,
                            str(i),
                            *[self.__val_check(temp_slice, x) for x in params_list],
                        ],
                        index=["patient", "slice#", *params_list],
                    )
                    dataset_stats = pd.concat(
                        [dataset_stats, new_row.to_frame().T], ignore_index=True
                    )
            ...
            for pat, path in tqdm(self, desc="Patients processed"):
                try:
                    ...
                    for j in z_ind:
                        ...
                        plt.imshow(temp_image_array[j, ...], cmap="bone")
                        plt.contour(
                            temp_mask_array[j, ...], colors="red", linewidths=2
                        )  # ,alpha=0.7)
                        plt.savefig(
                            os.path.join(directory, "slice #%d" % j),
                            bbox_inches="tight",
                        )
                        plt.close()
                except Exception:
    ...
    def extract_features(
        self, params_file: str, loggenabled: bool = False
    ) -> DataFrame:
            ...
            for pat, path in tqdm(self, desc="Patients processed"):
                try:
                    ...
                    if pat_features["diagnostics_Image-original_Hash"] != "":
                        pat_features.update(
                            {"Patient": pat, "ROI": path[1].split(os.sep)[-1][:-5]}
                        )
    ...
    def convert_to_nrrd(
        self, export_path: str, region_of_interest: str = "all", image_type=np.int16
    ):
        ...
        if self._data_type == "dcm":
            if self._image_only:
                ...
                for pat, pat_path in tqdm(self, desc="Patients converted"):
                    ...
                    rt_path = pat_path[1]
                    try:
                        rt_structure, roi_list = self.__get_roi(
                            region_of_interest, rt_path
                        )
                    except KeyboardInterrupt:
                        raise
                    except Exception:
                    ...
                    for roi in roi_list:
                        try:
                            image, mask = self.__get_binary_mask(
                                img_path, rt_structure, roi, image_type
                            )

                            export_dir = os.path.join(
                                export_path, "converted_nrrds", pat
                            )
                            ...
                            sitk.WriteImage(
                                image, os.path.join(export_dir, image_file_name)
                            )  # save image and binary mask locally
                            sitk.WriteImage(
                                mask, os.path.join(export_dir, mask_file_name)
                            )
                        except KeyboardInterrupt:
                            raise
                        except Exception:
    ...
    def pre_process(
        ...
        save_path: str = None,
        z_score: bool = False,
        norm_coeff: tuple = None,
        hist_match: bool = False,
        hist_equalize: bool = False,
        binning: bool = False,
        percentile_scaling: bool = False,
        corr_bias_field: bool = False,
        subcateneus_fat: bool = False,
        fat_value: bool = None,
        reshape: bool = False,
        to_shape: np.array = None,
        ...
        for i, pat in tqdm(self):
            ...
            pre_processed_arr = self.__preprocessing_function(
                image_array,
                mask_array,
                ref_image_arr,
                z_score,
            ...
            sitk.WriteImage(
                pre_processed_image, os.path.join(export_dir, "pre_processed_img.nrrd")
            )  # save image and binary mask locally
        ...
        params = {
            ...
            "binning": binning,
            "percentile_scaling": percentile_scaling,
            "subcateneus_fat": subcateneus_fat,
            "fat_value": fat_value,
            "verbosity": verbosity,
            "visualize": visualize,
        }
        ...
        with open(
            os.path.join(save_path, "pre-processing_parameters.csv"), "w"
        ) as csv_file:
    ...
    def get_quality_checks(
        ...
        df_columns = [
            ...
            "Scan len is in range",
            "Slice thickness is in range",
            "Slice thickness is consistent",
            "Pixel spacing is in range",
            "Convolutional kernel tag is present",
            "Convolutional kernel is acceptable",
        ...
        for pat in tqdm(self):
            ...
            checks_df = pd.concat([checks_df, checks.to_frame().T], ignore_index=True)
        return checks_df
    ...
    def __quality_checks(self, patient, path, qc_parameters, columns, verbosity):
        ...
        if qc_parameters.get("specific_modality", 0):
            if len(qc_parameters["specific_modality"]) > 0:
                try:
                    modality = self.__check_modality(
                        scans, qc_parameters["specific_modality"]
                    )
                    if verbosity:
        ...
        if qc_parameters.get("scan_length_range", 0):
            if len(qc_parameters["scan_length_range"]) == 2:
                try:
                    if (
                        len(scans) >= qc_parameters["scan_length_range"][0]
            ...
            if verbosity:
                print(
                    "Acceptable range was not provided for the number of slices check."
                )
        ...
        if qc_parameters.get("spacing_range", 0):
            if len(qc_parameters["spacing_range"]) == 2:
                try:
                    ...
                    if (
                        ps[0] == ps[1]
                        and ps[0] >= qc_parameters["spacing_range"][0]
                        and ps[0] <= qc_parameters["spacing_range"][1]
                    ):
                        pixel_sp = 1
                    else:
                        pixel_sp = 0
                    if verbosity:
                        print(
                            "Pixel spacing in acceptable range check status:", pixel_sp
                        )
                except Exception:
                    pixel_sp = np.nan
                    print(
                        "Cannot perform pixel spacing range check for pat: %s" % patient
                    )
            else:
                raise MyException(
            ...
            if verbosity:
                print(
                    "Acceptable range was not provided for the pixel spacing range check."
                )
        ...
        if qc_parameters.get("axial_res", 0):
            if len(qc_parameters["axial_res"]) == 2:
                try:
                    intercept_slope, image_shape = self.__get_pixel_values(
                        scans, np.int8, True, qc_parameters["axial_res"]
                    )
                    if verbosity:
                        print("Intercept/slope check status:", intercept_slope)
                        print(
                            "Image shape is %s check status"
                    ...
                    print(
                        "Cannot perform Intercept/slope and image shape check for pat: %s"
                        % patient
                    )
            else:
                raise MyException(
                    "Please correctly specify the axial_res in parameters (e.g. 'axial_res':[512,512])"
                )
        else:
            ...
            if len(qc_parameters["thickness_range"]) == 2:
                try:
                    slice_thickness_consistency, slice_thickness_range = (
                        self.__check_consistent_slice_thickness(
                            scans, thickness_range=qc_parameters["thickness_range"]
                        )
                    )
                    if verbosity:
                        print(
                            "Slice thickness consistency check status:",
                            slice_thickness_consistency,
                        )
                        print(
                            "Slice thickness in acceptable range check status:",
                            slice_thickness_range,
                        )
                except Exception:
                    slice_thickness_consistency, slice_thickness_range = np.nan, np.nan
                    print(
                        "Cannot perform slice thickness consistency and range check for pat: %s"
                        % patient
                    )
            else:
                raise MyException(
        ...
        try:
            ...
            if verbosity:
                print(
                    "Missing/overlapping slices check status:",
                    missing_overlapping_slices,
                )
        except Exception:
            missing_overlapping_slices = np.nan
            print(
                "Cannot perform missing/overlapping slice check for pat: %s" % patient
            )
        ...
        if qc_parameters.get("kernels_list", 0):
            if len(qc_parameters["kernels_list"]) > 0:
                try:
                    conv_kern = self.__check_kernel(
                        scans, qc_parameters["kernels_list"]
                    )
                    conv_pres = 1
                    if verbosity:
            ...
            if verbosity:
                print(
                    "Acceptable value was not provided for the 'kernels_list', conv kernel check was not performed "
                )
    ...
    def __check_image_axial_plane(self, img):
        if (
            int(img.ImageOrientationPatient[0])
            and int(img.ImageOrientationPatient[4])
            and not int(img.ImageOrientationPatient[1])
            and not int(img.ImageOrientationPatient[2])
            and not int(img.ImageOrientationPatient[3])
            and not int(img.ImageOrientationPatient[5])
        ):
            return 1
        else:
            return 0
    ...
    def __check_consistent_slice_thickness(self, scans, thickness_range):
        ...
        if slice_thickness.count(slice_thickness[0]) == len(slice_thickness):
            if (
                slice_thickness[0] >= thickness_range[0]
                and slice_thickness[1] <= thickness_range[1]
            ):
                return 1, 1
            else:
                return 1, 0
    ...
    def __check_missing_overlapping_slices(self, scans):
        temp_spacing = []
        scan_real_range = np.round(
            list(np.float(x.ImagePositionPatient[2]) for x in scans), 1
        )
        for i in range(len(scan_real_range) - 1):
            temp_spacing.append(
                np.round(scan_real_range[i + 1] - scan_real_range[i], 1)
            )
    ...
    def __check_kernel(self, scans, kernels=[]):
        ...
        if kernels_list.count(kernels_list[0]) == len(kernels_list) and kernels_list[
            0
        ].lower() in set(kernels):
            return 1
        else:
            return 0
    ...
    def __get_roi(self, region_of_interest, rt_path):
        ...
        if region_of_interest.lower() == "all":
            roi_list = [
                rt_structure.StructureSetROISequence[x].ROIName
                for x in range(0, len(rt_structure.StructureSetROISequence))
            ]
        else:
            for i in range(0, len(rt_structure.StructureSetROISequence)):
                if region_of_interest.lower()[0] == "!":
                    ...
                    if (
                        ...
                        roi_list.append(
                            rt_structure.StructureSetROISequence[i].ROIName
                        )  ## only roi with the exact same name

                elif re.search(
                    region_of_interest.lower(),
                    str(rt_structure.StructureSetROISequence[i].ROIName).lower(),
                ):
    ...
    def __read_scan(self, path):
        ...
        for s in os.listdir(path):
            try:
                ...
                temp_file.file_meta.TransferSyntaxUID = (
    ...
    def __get_pixel_values(self, scans, image_type, qa=False, image_res=[512, 512]):
        try:
            image = np.stack(
                [s.pixel_array * s.RescaleSlope + s.RescaleIntercept for s in scans]
            )
            if qa:
    ...
    def __get_image(self, img_path, image_type):
        ...
        zres = np.abs(
            image[1].ImagePositionPatient[2] - image[0].ImagePositionPatient[2]
        )
        ...
        image_sitk.SetOrigin(
            (
                float(img_first_slice.ImagePositionPatient[0]),
                float(img_first_slice.ImagePositionPatient[1]),
    ...
    def __get_binary_mask(self, img_path, rt_structure, roi, image_type):
        ...
        mask = np.zeros(
            [img_length, img_first_slice.Rows, img_first_slice.Columns], dtype=np.uint8
        )
        xres = np.array(img_first_slice.PixelSpacing[0])
        yres = np.array(img_first_slice.PixelSpacing[1])
        zres = np.abs(
        ...
        Fm = np.zeros((3, 2))
        Fm[:, 0], Fm[:, 1] = (
            img_first_slice.ImageOrientationPatient[:3],
            img_first_slice.ImageOrientationPatient[3:],
        )
        row_pr, column_pr = (
            img_first_slice.PixelSpacing[0],
            img_first_slice.PixelSpacing[1],
        )
        ...
        for sequence in rt_structure.ROIContourSequence[roi_id].ContourSequence:
            ...
            img_slice = self.__coordinates_to_mask(
                pixel_coords_c_r_s[1, :],
                pixel_coords_c_r_s[0, :],
    ...
    def __normalize_image_zscore_per_image(
        self, image, mask, verbosity
    ):  ##Zscore based on the masked region intensities
        image_s = image.copy()
    ...
    def __resample_intensities(
        self, orig_img, bin_nr, verbosity
    ):  ## Intensity resampling whole image/or masked region
        v_count = 0
        img_list = []
        ...
        print(
            "min: ",
            np.min(filtered.flatten()),
            "max: ",
            np.max(filtered.flatten()),
            "step: ",
            step,
        )
        ...
        if verbosity:
            print(
                "Resampling with a step of: ",
                step,
                "Amount of unique values, original img: ",
                len(np.unique(orig_img.flatten())),
                "resampled img: ",
                len(np.unique(resampled.flatten())),
            )
    ...
    def __resize_3d_img(self, img, shape, interp=cv2.INTER_CUBIC):
        ...
        for i in range(0, init_img.shape[0]):
            temp_img[i, ...] = cv2.resize(
                init_img[i, ...], dsize=(shape[2], shape[1]), interpolation=interp
            )
        for j in range(0, shape[1]):
            new_img[:, j, :] = cv2.resize(
                temp_img[:, j, :], dsize=(shape[2], shape[0]), interpolation=interp
            )
        return new_img
    ...
    def __correct_bias_field(self, img, mask, n_fitting_levels, n_iterations):
        ...
        img_output = sitk.GetArrayFromImage(
            corrector.Execute(img_sitk_cast, mask_sitk_cast)
        )
    ...
    def __preprocessing_function(
        ...
        if verbosity:
            ...
            print(
                "Number of unique intensity values :", len(unique_number_of_intensity)
            )
        ...
        if visualize:
            ...
            plt.imshow(img[int(len(img) / 2.0), ...], cmap="bone")
            plt.title("original image")
            plt.show()

        if corr_bias_field:
            img = self.__correct_bias_field(
                img, mask, n_fitting_levels=4, n_iterations=[50]
            )
            if verbosity:
        ...
        if hist_match:
            ...
            if visualize:
                ...
                plt.subplot(122)
                plt.imshow(ref_img[int(len(img) / 2.0), ...], cmap="bone")
                plt.title("Ref image")
                plt.show()
        ...
        if z_score and norm_coeff:
            ...
            if verbosity:
                print(
                    "Z-score normalization applied based on the whole image, Mu=%s, sigma=%s"
                    % (norm_coeff)
                )
            if visualize:
                plt.figure(figsize=(12, 12))
            ...
            if verbosity:
                print(
                    "Z-score normalization applied based on image intensities, Mu=%s, sigma=%s"
                    % (mu, sigma)
                )
            if visualize:
                plt.figure(figsize=(12, 12))
