...
is_in_list = lambda names_list, name: any(
    [True for v in names_list if re.search(v, name.lower())]
)
        ...
        if self._data_type == "nrrd":
            for patient in tqdm(self.__patients):
                ...
                for root, dirs, files in os.walk(
                    os.path.join(self._data_path, patient)
                ):
                    for file in files:
            ...
            for patient in tqdm(self.__patients):
                    ...
                    for root, dirs, files in os.walk(os.path.join(self._data_path)):
                        for file in files:
                            if file.endswith(".dcm"):
                                temp_file = pydicom.read_file(
                                    os.path.join(root, file), force=True
                                )
                ...
                if not len(dcm_files):
                    warn(
                        "No dcm data found for patient:%s check the folder, ensure that dicom files ends with .dcm"
                        % patient
                    )
                ...
                if messy_structure:
                    for rt in all_dicom_subfiles:
                        ...
                        for dfile in dcm_files:
                            ...
                            if is_in_list(
                                ["ct", "mr", "us", "nm"],
                                str(temp_dfile.Modality).lower(),
                            ):
                                if (
                                    temp_dfile.SeriesInstanceUID
                                    != temp_rt.SeriesInstanceUID
                                ):
                                    pass
                                else:
                                    datafile = os.path.dirname(os.path.abspath(dfile))
                                    rts_name = rt[:-4].split(os.sep)[
                                        -2
                                    ]  # +rt[:-4].split(os.sep)[-2]
                                    self._patient_dict[
                                        patient + "_" + rts_name[-15:]
                                    ] = [datafile, rt]
                                    break
                else:
                    for file in dcm_files:
                                ...
                                if temp_file.Modality == "RTSTRUCT":
                                    ...
                                    for dfile in dcm_files:
                                        ...
                                        if is_in_list(
                                            ...
                                            if (
                                                temp_dfile.StudyInstanceUID
                                                != temp_file.StudyInstanceUID
                                            ):
                                                warn("StudyInstanceUID doesnt match!")

                                            datafile = os.path.dirname(
                                                os.path.abspath(dfile)
                                            )
                        ...
                        if not self.__multi_rts_per_pat and dict_length - len(
                            self._patient_dict
                        ):
                            break
            ...
            raise NotImplementedError(
                'Currently only "dcm" format and "nrrd" (nrrd/mha) formats are supported'
            )
