...
class GenerateResultBox(ResultSet):
    ...
    def get_optimal_threshold(self) -> float:
        ...
        fpr, tpr, thresholds = sklearn.metrics.roc_curve(
            self.train_df["labels"], self.train_df["predictions"]
        )
    ...
    def _linking_data(self, label) -> list:
        ...
        if label == "train":
            y_label, y_pred = (
                self.train_df["labels"].values,
                self.train_df["predictions"].values,
            )
        if label == "test":
            y_label, y_pred = (
                self.test_df["labels"].values,
                self.test_df["predictions"].values,
            )
        if label == "external":
            y_label, y_pred = (
                self.external_df["labels"].values,
                self.external_df["predictions"].values,
            )
    ...
    def get_results(self, label: str = None) -> DataFrame:
        ...
        dict_results["balanced accuracy"] = [
            sklearn.metrics.balanced_accuracy_score(y_label, y_pred_binary)
        ]
        dict_results["precision"] = [
            sklearn.metrics.precision_score(y_label, y_pred_binary)
        ]
    ...
    def _get_ci(self, y_label, y_pred, f, nsamples):
        ...
        return [
            "%0.2f CI [%0.2f,%0.2f]" % (f(y_label, y_pred), ci[0], ci[1])
    ...
    def _get_ci_for_auc(self, y_label, y_pred, nsamples):
        ...
        for b in range(nsamples):
            ...
            temp_fpr, temp_tpr, temp_thresholds = sklearn.metrics.roc_curve(
                y_label[idx], temp_pred
            )
        ...
        return [
            "%0.2f CI [%0.2f,%0.2f]"
            % (sklearn.metrics.auc(fpr, tpr), ci_auc[0], ci_auc[1])
        ]
    ...
    def get_stats_with_ci(self, label: str = None, nsamples: int = 2000) -> DataFrame:
        ...
        if self._threshold == -1:
            self.get_optimal_threshold()
        y_pred_binary = (np.array(y_pred) > self._threshold).astype(int)
        dict_results["balanced accuracy"] = self._get_ci(
            y_label, y_pred_binary, sklearn.metrics.balanced_accuracy_score, nsamples
        )
        dict_results["precision"] = self._get_ci(
            y_label, y_pred_binary, sklearn.metrics.precision_score, nsamples
        )
        dict_results["recall"] = self._get_ci(
            y_label, y_pred_binary, sklearn.metrics.recall_score, nsamples
        )
        dict_results["f1 score"] = self._get_ci(
            y_label, y_pred_binary, sklearn.metrics.f1_score, nsamples
        )
        df_results = pd.DataFrame.from_dict(dict_results)
        df_results = df_results.reset_index(drop=True)
        df_results.index = [label]
    ...
    def plot_roc_auc_ci(
        self, title: str = "", nsamples: int = 2000, save_fig: bool = False
    ) -> str:
        """
        ...
        if "train" in curves_to_plot:
            ...
            for b in range(nsamples):
                idx_train = np.random.randint(
                    np.array(list(true_train_outcome)).shape[0],
            ...
            mean_auc_train = sklearn.metrics.auc(mean_fpr_train, mean_tpr_train)
            ci_auc_train = np.percentile(auc_values_train, (2.5, 97.5))

        if "external" in curves_to_plot:
            true_external_binary_outcome, proba_external = self._linking_data(
                "external"
            )
            auc_values_external = []
            tprs_external = []
            ...
            for b in range(nsamples):
                idx_external = np.random.randint(
                    np.array(list(true_external_binary_outcome)).shape[0],
                    size=np.array(list(true_external_binary_outcome)).shape[0],
                )
                pred = proba_external[idx_external]
                fpr_external, tpr_external, thresholds_external = (
                    sklearn.metrics.roc_curve(
                        np.array(list(true_external_binary_outcome))[idx_external], pred
                    )
                )
                interp_tpr_external = np.interp(
                    mean_fpr_external, fpr_external, tpr_external
                )
            ...
            mean_auc_external = sklearn.metrics.auc(
                mean_fpr_external, mean_tpr_external
            )  # does mean auc makes sense or should it be from the overall predictions?
        ...
        if "test" in curves_to_plot:
            ...
            for b in range(nsamples):
                idx_test = np.random.randint(
                    np.array(list(true_test_outcome)).shape[0],
        ...
        if "train" in curves_to_plot:
            ...
            interval_tprs_train = np.array(
                [
                    np.percentile(np.array(tprs_train)[:, i], (2.5, 97.5))
                    for i in range(100)
                ]
            )
        ...
        if "test" in curves_to_plot:
            ax.plot(
                ...
                mean_tpr,
                color="blue",
                label=r"Mean ROC test (AUC = %0.2f CI [%0.2f,%0.2f])"
                % (mean_auc, ci_auc[0], ci_auc[1]),
                lw=2,
                alpha=0.8,
            )
            interval_tprs = np.array(
                [np.percentile(np.array(tprs)[:, i], (2.5, 97.5)) for i in range(100)]
            )
            tprs_upper = interval_tprs[:, 1]
            tprs_lower = interval_tprs[:, 0]
            ax.fill_between(
                mean_fpr, tprs_lower, tprs_upper, color="darkblue", alpha=0.3
            )
        ...
        if "external" in curves_to_plot:
            ...
            interval_tprs_external = np.array(
                [
                    np.percentile(np.array(tprs_external)[:, i], (2.5, 97.5))
                    for i in range(100)
                ]
            )
    ...
    def print_confusion_matrix(
        ...
        try:
            if normalize:
                fmt = ".2f"
                heatmap = sns.heatmap(
                    df_cm, annot=True, fmt=fmt, cmap="Blues", vmin=0, vmax=1
                )
            else:
                fmt = "d"
                heatmap = sns.heatmap(
        ...
        if save_fig:
            if normalize:
                plt.savefig(
                    "normalized confusion matrices on the "
                    + label
                    + " "
                    + class_names[0]
                    + " vs "
