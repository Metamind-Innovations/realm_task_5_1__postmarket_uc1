...
class AnalysisBox(FeaturesSet):
    ...
    def handle_nan(self, axis: int = 1, how: str = "any", mode: str = "delete"):
        ...
        if mode == "delete":
            ...
            for label_name in self._class_label:
                data_balance.append(
                    np.sum(np.array(list(self._outcome)) == label_name)
                    / len(self._outcome)
                )
            print(
                "Number of observations: {}\nClass labels: {}\nClasses balance: {}".format(
                    len(self._outcome), self._class_label, data_balance
                )
            )
        if mode == "fill":
    ...
    def handle_constant(self):
        ...
        constant_features = self._feature_dataframe.columns[
            self._feature_dataframe.nunique() <= 1
        ]
        ...
        for label_name in self._class_label:
            data_balance.append(
                np.sum(np.array(list(self._outcome)) == label_name) / len(self._outcome)
            )
        print(
            "Number of observations: {}\nClass labels: {}\nClasses balance: {}".format(
                len(self._outcome), self._class_label, data_balance
    ...
    def normality_check(
        self, features_to_plot: list = [], p_thresh: float = 0.05
    ) -> list:
        """
        ...
        for feature in num_features:
            shapiro_test = sp.stats.shapiro(
                np.array(list(self._feature_dataframe[feature]))
            ).pvalue
    ...
    def plot_distribution(
        self, features_to_plot: list = [], binary_classes_to_plot: list = []
    ):
        ...
        if len(self._outcome) > 0:
            if len(binary_classes_to_plot) == 2:
                if (binary_classes_to_plot[0] in self._class_label) & (
                    ...
                    fig = plotly.subplots.make_subplots(
                        rows=rows, cols=cols, subplot_titles=num_features_tuple
                    )
                    counter = 0
                    for feature in num_features:
                        c = counter % 4 + 1
                        ...
                        fig.append_trace(
                            go.Histogram(
                                x=self._feature_outcome_dataframe.loc[
                                    self._feature_outcome_dataframe[
                    ...
                    for i in fig["layout"]["annotations"]:
                        i["font"] = dict(size=10)
                    fig.update_layout(
                        title_text="Features binary distribution in classes",
                        height=rows * 250,
                        width=1250,
                    )
                    plotly.offline.plot(
                        fig,
                        filename=os.path.splitext(self._feature_path)[0]
                        + "_distr.html",
    ...
    def plot_correlation_matrix(
        ...
        fig.update_layout(
            height=len(num_features) * 20 + 250, width=len(num_features) * 20 + 250
        )
        plotly.offline.plot(
            fig,
        ...
        if save_to_csv:
            corr_matrix_df.to_csv(
                os.path.splitext(self._feature_path)[0] + "_correlation_matrix.csv"
            )
    ...
    def __get_MW_p(self, ftrs, binary_classes_to_plot, p_threshold=0.05):
        p_MW = []
        df_0 = self._feature_outcome_dataframe.loc[
            self._feature_outcome_dataframe[self._outcome_column]
            == binary_classes_to_plot[0]
        ]
        df_1 = self._feature_outcome_dataframe.loc[
            self._feature_outcome_dataframe[self._outcome_column]
            == binary_classes_to_plot[1]
        ]
        for feature in ftrs:
            s, p = sp.stats.mannwhitneyu(
                df_0[feature].astype(float),
                df_1[feature].astype(float),
                alternative="two-sided",
            )
            p_MW.append(p)
        reject, p_MW_corr, a_S_corr, a_B_corr = multitest.multipletests(
            p_MW, alpha=p_threshold, method="bonferroni"
        )
        return p_MW_corr
        ...
        if len(self._outcome) > 0:
            if len(self._class_label) == 2:
                ...
                if len(binary_classes_to_plot) != 2:
                    ...
                    (binary_classes_to_plot[0] in self._class_label)
                    & (binary_classes_to_plot[1] in self._class_label)
                ):
                    print("Wrong class label(s).")
                    return
            ...
            fig.update_xaxes(tickfont=dict(size=7))
            fig.update_layout(
                title_text="The p-values for Mann-Whitney test (Bonferroni corrected)",
    ...
    def __get_univar_fprs_tprs(self, ftr, binary_classes_to_plot):
        mean_0 = self._feature_outcome_dataframe.loc[
            self._feature_outcome_dataframe[self._outcome_column]
            == binary_classes_to_plot[0]
        ][ftr].mean()
        mean_1 = self._feature_outcome_dataframe.loc[
            self._feature_outcome_dataframe[self._outcome_column]
            == binary_classes_to_plot[1]
        ][ftr].mean()
        ...
        n_0 = len(
            self._feature_outcome_dataframe.loc[
                self._feature_outcome_dataframe[self._outcome_column]
                == binary_classes_to_plot[0]
            ]
        )
    ...
    def plot_univariate_roc(
        ...
        if len(self._outcome) > 0:
            ...
            for feature in num_features:
                c = counter % 4 + 1
                r = counter // 4 + 1
                fprs, tprs = self.__get_univar_fprs_tprs(
                    ftr=feature, binary_classes_to_plot=binary_classes_to_plot
                )
            ...
            fig.update_layout(
                title_text="Features univariate ROC-curves:"
                + str(binary_classes_to_plot),
                height=rows * 250,
                width=1250,
                showlegend=False,
            )
    ...
    def calculate_basic_stats(self, volume_feature: str = ""):
        ...
        frame = {
            "NaN": self._feature_dataframe[num_features].isnull().sum(axis=0),
        ...
        if len(self._outcome) > 0:
            if len(self._class_label) == 2:
                p_MW_corr = self.__get_MW_p(
                    ftrs=num_features, binary_classes_to_plot=self._class_label
                )
                univar_auc = []
                for feature in num_features:
                    fprs, tprs = self.__get_univar_fprs_tprs(
                        ftr=feature, binary_classes_to_plot=self._class_label
                    )
                    univar_auc.append(auc(fprs, tprs))
                stats_dataframe_ext = pd.DataFrame(
                    {"p_MW_corrected": p_MW_corr, "univar_auc": univar_auc},
                    index=num_features,
                )

                stats_dataframe = pd.concat(
                    [stats_dataframe, stats_dataframe_ext], axis=1
                )
        ...
        if volume_feature:
            ...
            for feature in num_features:
                vol_corr.append(
                    sp.stats.spearmanr(
                        self._feature_dataframe[feature],
                        self._feature_dataframe[volume_feature],
                    )[0]
                )
            stats_dataframe = pd.concat(
                [
        ...
        stats_dataframe.to_excel(
            os.path.splitext(self._feature_path)[0] + "_basic_stats.xlsx"
        )
        ...
        if volume_feature:
            if volume_feature in self._feature_column:
                    ...
                    if len(self._class_label) == 2:
                        ...
                        fig.add_trace(
                            go.Scatter(
                                ...
                                name="PRC",
                                marker={
                                    "color": self.__get_color(univar_auc, auc_threshold)
                                },
                            )
                        )
                        fig.add_trace(
                        ...
                        fig.update_layout(
                            title_text="Volume precision-recall curve",
                            showlegend=False,
                            width=500,
                            height=500,
                        )
                ...
                fig.update_layout(
                    ...
                    title_text="Volume Spearman correlation",
                    showlegend=False,
                    width=750,
                    height=len(num_features) * 20 + 250,
                )
